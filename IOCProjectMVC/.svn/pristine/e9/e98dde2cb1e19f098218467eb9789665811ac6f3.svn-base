    ...
    JSON, on, parser, arrayUtils, lang, CsvStore, base64
     ...
     // on(dom.byId("clearButton"), "click", clearAll);
          ...
          reader.onload = function () {
              ...
              img.onload = function () {
                  ...
                  var symbol = new PictureMarkerSymbol(reader.result,
                    width > 64 ? 64 : width,
                    height > 64 ? 64 : height);
                  var point = PropiedadesOwnData.map.toMap(new Point(x, y));
                  var graphic = new Graphic(point, symbol);
                  PropiedadesOwnData.map.graphics.add(graphic);
              };
      ...
      function handleMapServer(url) {
          ...
          PropiedadesOwnData.map.addLayer(layer);
      }
      ...
      function processCSVData(data) {
          ...
          csvStore.fetch({
              onComplete: function (items) {
                  ...
                  PropiedadesOwnData.map.addLayer(featureLayer);
                  zoomToData(featureLayer);

                  //Función que rellena el panel con los campos de la capa csv para que el usuario elija por que campos quiere construir el gráfico
                  fillPanelCSVLayerFields();
      ...
      //Función que rellena el panel con los campos de la capa csv para que el usuario elija por que campos quiere construir el gráfico
      function fillPanelCSVLayerFields() {
          var esritypes = ["esriFieldTypeSmallInteger", "esriFieldTypeInteger", "esriFieldTypeSingle", "esriFieldTypeDouble"];
          $.each(PropiedadesOwnData.map.getLayer("csvLayer").fields, function (index, value) {
      ...
      function clearAll() {
          PropiedadesOwnData.map.graphics.clear();
          //var layerIds = PropiedadesOwnData.map.graphicsLayerIds.slice(0);
          //layerIds = layerIds.concat(PropiedadesOwnData.map.layerIds.slice(1));

          arrayUtils.forEach(PropiedadesOwnData.LayersToClear, function (layerId) {
              //console.log(PropiedadesOwnData.map.getLayer(layerId));
              (PropiedadesOwnData.map.getLayer(layerId)) ? PropiedadesOwnData.map.removeLayer(PropiedadesOwnData.map.getLayer(layerId)) : "";
              //PropiedadesOwnData.map.removeLayer(PropiedadesOwnData.map.getLayer(layerId));
          });
      }
      ...
      function zoomToData(featureLayer) {
          ...
          var multipoint = new Multipoint(PropiedadesOwnData.map.spatialReference);
          ...
          if (multipoint.points.length > 0) {
              PropiedadesOwnData.map.setExtent(multipoint.getExtent().expand(1.25), true);
          }
      }
